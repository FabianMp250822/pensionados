...
// Función para reemplazar los placeholders en la plantilla y aplicar formato en negrilla a los datos dinámicos
const generarDocumento = (plantilla, datos) => {
  let documento = plantilla;
  Object.keys(datos).forEach((key) => {
    const regex = new RegExp(`{{${key}}}`, 'g');
    documento = documento.replace(regex, `<strong>${datos[key]}</strong>`);
  });
...
- El Mandante está domiciliado en CALLE 79B 42 540, APTO 103. Tel.: 3157899677. Email: ANDIGRA@GMAIL.COM
...
const ContratoPrestacionServicios = ({ onClose, cliente }) => {
  const [loading, setLoading] = useState(false);

  // Si no se recibe cliente, mostramos un mensaje de error
  if (!cliente) {
    return (
      <div style={{ padding: '20px', textAlign: 'center' }}>
        <p>No se han recibido los datos del cliente.</p>
        <button onClick={onClose} style={{ padding: '10px 20px', border: 'none', borderRadius: '6px', backgroundColor: '#007bff', color: '#fff', cursor: 'pointer' }}>
          Cerrar
        </button>
      </div>
    );
  ...
  // Armamos los datos para reemplazar en la plantilla
  const datosContrato = {
    mandanteNombre: `${cliente.nombres} ${cliente.apellidos}`,
    mandanteCedula: cliente.cedula,
    electrantaNombre: cliente.electrantaNombre || 'Nombre de Electrificadora'
  };

  // Generamos el documento del contrato
  const documentoContrato = generarDocumento(plantillaContrato, datosContrato);
  ...
  // Función para guardar el contrato en la subcolección "contrato" del documento en "nuevosclientes"
  const handleGuardarContrato = async () => {
    setLoading(true);
    try {
      const db = getFirestore();
      const contratoCollectionRef = collection(db, "nuevosclientes", cliente.id, "contrato");
      await addDoc(contratoCollectionRef, {
        correoDestinatario: cliente.correo,
        nombreMandante: datosContrato.mandanteNombre,
        documentoContrato: documentoContrato,
        createdAt: serverTimestamp()
      });
      alert("Contrato guardado exitosamente en la subcolección 'contrato'.");
    } catch (error) {
      console.error("Error guardando el contrato:", error);
      alert("Error guardando el contrato.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div style={containerStyle}>
      ...
      <div style={headerStyle}>
        <h1 style={titleStyle}>Contrato de Prestación de Servicios</h1>
      </div>

      {/* Contenido del contrato */}
      <div style={contentStyle} dangerouslySetInnerHTML={{ __html: documentoContrato }} />
...
export default ContratoPrestacionServicios;
